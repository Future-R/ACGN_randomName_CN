...
[assembly: XQPlugin("二次元起名器", "未来", "1.0.3", "味儿很冲の二次元作品名&网名随机作成！")]
namespace 二次元起名器
{
    [Plugin]
    public class Root
    {
        ...
        static string[] 默认后缀
        {
            get
            {
                return new[] { "代码", "计划", "指令", "结界", "领域", "地带", "力量", "王冠", "次元", "风暴", "黎明", "地狱", "枷锁", "梦境", "轮回", "沙漏", "残响", "之花", "之魂", "之翼", "之心", "默示录", "序曲", "终章", "诗篇", "战车", "阳炎", "红炎", "失格", "宣告", "审判", "契约", "乐队", "杀手", "使徒", "统领", "纹章", "深渊", "鸟居", "特攻", "游戏", "英雄", "余晖", "纪元", "核心", "武装", "战争", "彼端", "教会", "禁止", "三日月", "圆舞曲", "协奏曲", "奏鸣曲", "狂想曲", "华尔兹", "理想乡", "决战兵器" };
            }
        }
        ...
        static string 随机组合()
        {
            ...
            if (new Random(Guid.NewGuid().GetHashCode()).Next(0, 100) < 5)
            {
                ...
                return 代号 + 默认后缀[后缀序号] + 星星 + 默认前缀[前缀序号] + 物语;
            }
            //正常输出
            ...
            return 代号 + 默认前缀[前缀序号] + 星星 + 默认后缀[后缀序号] + 物语;
        }
    }
}