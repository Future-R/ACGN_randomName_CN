...
[assembly: XQPlugin("二次元起名器", "未来", "1.0.2", "味儿很冲の二次元作品名&网名随机作成！")]
namespace 二次元起名器
{
    [Plugin]
    public class Root
    {
        static string[] 默认前缀
        {
            get
            {
                return new[] { "少女", "旅人", "勇者", "魔神", "终焉", "灰烬", "空想", "弑神", "奥术", "虚空", "神授", "诸神", "灵子", "不灭", "湮灭", "超载", "奇迹", "共鸣", "嗜血", "灵魂", "灾厄", "究极", "超越", "光明", "混沌", "希望", "绝望", "灭亡", "无限", "悲鸣", "原初", "心象", "星屑", "白夜", "零点", "绝对", "幻想", "沉默", "熔解", "弥生", "水无", "千年", "暴走", "青枫", "浮世", "胧月", "红莲", "黄泉", "鬼杀", "妖怪", "人间", "伊甸", "银河", "破碎", "真理", "空想", "天命", "死亡", "爆裂", "异邦", "の用户昵称" };
            }
        }
        static string[] 默认后缀
        {
            get
            {
                return new[] { "代码", "计划", "指令", "结界", "领域", "地带", "力量", "王冠", "次元", "风暴", "黎明", "地狱", "枷锁", "梦境", "轮回", "沙漏", "残响", "之花", "之魂", "之翼", "之心", "默示录", "序曲", "终章", "诗篇", "战车", "阳炎", "红炎", "失格", "宣告", "审判", "契约", "乐队", "杀手", "使徒", "统领", "纹章", "深渊", "鸟居", "特攻", "游戏", "英雄", "余晖", "纪元", "核心", "武装", "战争", "彼端", "教会", "三日月", "圆舞曲", "协奏曲", "奏鸣曲", "狂想曲", "华尔兹", "理想乡", "决战兵器" };
            }
        }
        ...
        static string 随机组合()
        {
            ...
            string 代号 = "";
            string 物语 = "";
            switch (new Random(Guid.NewGuid().GetHashCode()).Next(0, 100))
            {
                case 1:
                case 2:
                case 3:
            ...
            if (new Random(Guid.NewGuid().GetHashCode()).Next(0, 100) < 5)
            {
                ...
                {
                    return 代号 + 默认后缀[后缀序号].Substring(1) + "の" + 默认前缀[前缀序号] + 物语;
                }
                return 代号 + 默认后缀[后缀序号] + 默认前缀[前缀序号] + 物语;
            }
            //正常输出
            if (默认前缀[前缀序号].StartsWith("の") || 默认前缀[前缀序号].StartsWith("之"))
            {
                return 代号 + 默认前缀[前缀序号].Substring(1) + "の" + 默认后缀[后缀序号] + 物语;
            }
            return 代号 + 默认前缀[前缀序号] + 默认后缀[后缀序号] + 物语;
        }
    }
}